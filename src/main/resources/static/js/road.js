kakao.maps.load(function() {
	...
	var safetyImageSize = new kakao.maps.Size(30, 25);
	...
	/*  safetyIndex.forEach(function(safety) {
			...
			if (safety.region == "seoul") {
				safetyLat = 37.5665;
				safetyLon = 126.9780;
			} else if (safety.region == "gangwon") {
				safetyLat = 37.8855;
				safetyLon = 127.7298;
			} else if (safety.region == "chungcheong") {
				safetyLat = 36.6356;
				safetyLon = 127.4912;
			} else if (safety.region == "jeolla") {
				safetyLat = 35.7175;
				safetyLon = 126.8931;
			} else if (safety.region == "gyeongsang") {
				safetyLat = 35.2394;
				safetyLon = 128.6924;
			}
  
			if (safety.safetyScore > 80) {
				markerImage = safetyMarkerImageSafe;
			} else if (safety.safetyScore > 40) {
				markerImage = safetyMarkerImageCaution;
			} else {
				markerImage = safetyMarkerImageDanger;
			}
  
			var marker = new kakao.maps.Marker({
				position: new kakao.maps.LatLng(safetyLat, safetyLon),
				image: markerImage,
				map: null
			});
			...
			overlayContent.innerHTML = `
							<div class="info">
									...
									<div class="body">
											<div class="desc">
												<div class="title">${safety.safetyScore}</div>
											</div>
									</div>
							</div>`;
	...
	//const nationwideBtn = document.getElementById("nationwide");
	...
	safetyBtn.addEventListener("click", function() {
		...
		//activatedMarkers.safety = !activatedMarkers.safety;
/*		if (activatedMarkers.safety) {
			accidentMarkers.accident = false;
			accidentMarkers.construction = false;
			accidentMarkers.weather = false;
			accidentMarkers.cctv = false;
			accidentMarkers.freezing = false;
			accidentMarkers.tunnel = false;
			activatedMarkers.safety = false;
			safetyBtn.classList.remove("active");
			hideAllMarkers();
		} else {
			hideAllMarkers();
			activatedMarkers.safety = true;
    ...
    if (isSafetyVisible) {
			...
			accidentMarkers.tunnel = false;
			safetyBtn.classList.add("active");
			//enableAllRegionButtons();
		} else {
			hideAllMarkers();
			safetyBtn.classList.remove("active");
			//disableAllRegionButtons();
			...
			if (!isSafetyVisible) {
				...
				if (region === "nationwide") {
					if (isNationwideVisible) {
						...
						//showMarkers("nationwide", avgSafetyMarkers);
					...
					if (currentRegion === region) {
						...
						if (!data) {
							console.error("❌ 오류: 해당 지역의 좌표 정보가 없습니다.", currentRegion);
							return;
						}
						...
						//showMarkers(region, allSafetyMarkers);
	...
	/*	regionButtons.forEach(button => {
			button.addEventListener("click", function() {
				...
				console.log(isNationwideVisible);
				if (!isSafetyVisible) {
					var data = regionData[region];
					...
					if (currentRegion === region && !isNationwideVisible) {
						isNationwideVisible = false;
						currentRegion = null;
						console.log(isNationwideVisible);
						map.setLevel(regionData.nationwide.level);
						...
						showMarkers(null, allSafetyMarkers);
	...
	/*  nationwideBtn.addEventListener("click", function() {
...
/*	function createNationwideOverlay() {
		...
		avgSafetyIndex.forEach(data => {
			...
			overlayContent.innerHTML = `
              <div class="overlay-content">
                  <strong>${data.region.toUpperCase()} 평균 안전지수</strong>
	...
	/*  function disableAllRegionButtons() {
			regionButtons.forEach(button => {
				button.disabled = true;
				button.classList.remove("active");
			});
			nationwideBtn.disabled = true;
			nationwideBtn.classList.remove("active");
		}
	
		function enableAllRegionButtons() {
			regionButtons.forEach(button => {
				button.disabled = false;
				button.classList.add("active");
			});
			nationwideBtn.disabled = false;
			nationwideBtn.classList.add("active");
		}*/
	...
	function hideAllMarkers(exceptType = null) {
		...
		allSafetyMarkers.forEach(marker => marker.setMap(null));
		avgSafetyMarkers.forEach(marker => marker.setMap(null));
		regionOverlays.forEach(marker => marker.setMap(null));
	}
	...
	function setMarkers(type, show, region) {
		...
		switch (type) {
			...
			case "nationwide":
				...
				regionOverlays.forEach(marker => marker.setMap(show ? map : null));
				break;
			case "accident":
	...
	// 버튼 이벤트 등록
	//document.getElementById("nationwide").addEventListener("click", () => toggleSafetyMarkers("nationwide"));
	//document.getElementById("seoul").addEventListener("click", () => toggleSafetyMarkers("seoul"));
	//document.getElementById("gangwon").addEventListener("click", () => toggleSafetyMarkers("gangwon"));
	//document.getElementById("chungcheong").addEventListener("click", () => toggleSafetyMarkers("chungcheong"));
	//document.getElementById("jeolla").addEventListener("click", () => toggleSafetyMarkers("jeolla"));
	//document.getElementById("gyeongsang").addEventListener("click", () => toggleSafetyMarkers("gyeongsang"));

	//document.getElementById("safetyBtn").addEventListener("click", () => toggleMarkers("safety"));
