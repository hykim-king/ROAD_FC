...
/*	safetyIndex.forEach(function(safety) {
	...
	var currentRegion = null; // ✅ 현재 선택된 지역 (초기값: null)
	var allSafetyMarkers = []; // 전체 마커 저장
	var avgSafetyMarkers = []; // 지역별 평균 마커 저장
	...
	// ✅ 지역 버튼 클릭 시
	function toggleSafetyMarkers(region) {
		if (currentRegion === region) {
			// ✅ 같은 지역을 다시 클릭하면 줌아웃 & 마커 숨기기
			currentRegion = null;
			...
			showMarkers(null, allSafetyMarkers); // 모든 마커 숨김
			map.setDraggable(true);
			map.setZoomable(true);
		} else {
			// ✅ 지역 클릭 시 해당 지역으로 줌인 & 전체 마커 표시 & 줌 제한
			...
			map.setDraggable(true); // 지도 이동 가능
			map.setZoomable(false); // 확대 불가능
		}
	}

	// ✅ 전국 버튼 클릭 시: 전국으로 확대 & 5개 지역 마커만 표시 & 이동/줌 가능
	document.getElementById("nationwide").addEventListener("click", () => {
/*		if (currentRegion === "nationwide") {
			// ✅ 전국 버튼을 다시 누르면 마커 숨기기 & 이동/줌 가능
			currentRegion = null;
			showMarkers(null, avgSafetyMarkers); // ✅ 모든 마커 숨김
			map.setDraggable(true);
			map.setZoomable(true);
		} else {
			// ✅ 전국 버튼 클릭 시 전국 확대 & 5개 지역 마커만 표시 & 이동/줌 가능*/
	...
	function setMarkers(type, show) {
		...
		switch (type) {
			case "safety":
				safetyMarkers.forEach(marker => marker.setMap(show ? map : null));
				break;
			case "accident":
