...
public interface NoticeQuestionRepository extends JpaRepository<NoticeQuestion, Integer> {
	...
	@Query(value = "SELECT *	" + "  FROM Noticequestion 	" + " WHERE subject LIKE %:subject%", nativeQuery = true)
	...
	Optional<NoticeQuestion> findTopByIdLessThanOrderByIdDesc(Integer id); // 이전글

	Optional<NoticeQuestion> findTopByIdGreaterThanOrderByIdAsc(Integer id); // 다음글
